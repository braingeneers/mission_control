...
proxy override, IAM mount, issuer environment variables, and end-to-end test
steps.
...
Use the new host for MCP-only OIDC broker work. Leave the existing
`auth.braingeneers.gi.ucsc.edu` flow attached to `oauth2-proxy` and the current
web Auth0 login stack.

Broker artifacts live in:

- [`oauth2-broker/`](/home/davidparks21/myprojects/Braingeneers/mission_control/oauth2-broker)
